...
namespace mimir {
    ...
    public:
        ...
        HLIOAdvice() : Advice(AdviceType(PrimaryAdviceType::SOFTWARE_HLIO_LIB,
                       ...
                       _name(), _read_distribution(), _write_distribution(), _access_pattern(),
                       _file_advice_map() {}
        HLIOAdvice(const HLIOAdvice& other): Advice(other), _logical_representation(other._logical_representation),
                                             _name(other._name), _read_distribution(other._read_distribution),
                                             _write_distribution(other._write_distribution),
                                             _access_pattern(other._access_pattern), _file_advice_map(other._file_advice_map) {}
        HLIOAdvice(const HLIOAdvice&& other): Advice(other), _logical_representation(other._logical_representation),
                                              _name(other._name), _read_distribution(other._read_distribution),
                                              _write_distribution(other._write_distribution),
                                              _access_pattern(other._access_pattern), _file_advice_map(other._file_advice_map)  {}
