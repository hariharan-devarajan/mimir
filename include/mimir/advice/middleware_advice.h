...
namespace mimir {
    ...
    public:
        ...
        MiddlewareAdvice(AdviceType type) : Advice(type), _num_cpu_cores_used(1),
                                          _num_gpus_used(0),
                                          _ts_distribution(), _interfaces_used(),
                                          _file_access_pattern() {}

        MiddlewareAdvice() : Advice(AdviceType(PrimaryAdviceType::SOFTWARE_MIDDLEWARE_LIBRARY,
                                                     OperationAdviceType::NO_OP)) , _num_cpu_cores_used(1),
                             _num_gpus_used(0),
                             _ts_distribution(), _interfaces_used(),
                             _file_access_pattern() {}
        MiddlewareAdvice(const MiddlewareAdvice& other): Advice(other), _num_cpu_cores_used(other._num_cpu_cores_used),
