...
TEST_CASE("Write",
  ...
  for (size_t i = 0; i < args.iteration; ++i) {
    io.resumeTime();
    size_t bytes_written = fwrite(write_data.data(), sizeof(char), args.request_size, fh);
    io.pauseTime();
    REQUIRE(bytes_written == args.request_size);
  }
  finalization.resumeTime();
  auto new_file = get_filename(fh->_fileno);
  finalization.pauseTime();
  metadata.resumeTime();
  int close_status = fclose(fh);
  metadata.pauseTime();
...
TEST_CASE("Read",
  ...
  for (size_t i = 0; i < args.iteration; ++i) {
    io.resumeTime();
    size_t bytes_read = fread(read_data.data(), sizeof(char), args.request_size, fd);
    io.pauseTime();
    REQUIRE(bytes_read == args.request_size);
  }

  finalization.resumeTime();
  auto new_file = get_filename(fd->_fileno);
  finalization.pauseTime();
...
TEST_CASE("ReadAfterWrite",
  ...
  metadata.pauseTime();
  REQUIRE(write_fd != nullptr);

  for (size_t i = 0; i < args.iteration; ++i) {
    io.resumeTime();
    size_t bytes_written =
        fwrite(write_data.data(), sizeof(char), args.request_size, write_fd);
    io.pauseTime();
    REQUIRE(bytes_written == args.request_size);
  }
  finalization.resumeTime();
  auto new_file_write = get_filename(write_fd->_fileno);
  finalization.pauseTime();
  metadata.resumeTime();
  int close_status_write = fclose(write_fd);
  metadata.pauseTime();
  ...
  /** Read I/O **/
  metadata.resumeTime();
  FILE* read_fd = fopen(filepath.c_str(), "r");
  metadata.pauseTime();
  REQUIRE(read_fd != nullptr);

  for (size_t i = 0; i < args.iteration; ++i) {
    io.resumeTime();
    size_t bytes_read = fread(read_data.data(), sizeof(char), args.request_size, read_fd);
    io.pauseTime();
    REQUIRE(bytes_read == args.request_size);
  }

  finalization.resumeTime();
  auto new_file_read = get_filename(read_fd->_fileno);
  finalization.pauseTime();

  metadata.resumeTime();
  int close_status = fclose(read_fd);
  metadata.pauseTime();
  REQUIRE(close_status == 0);
...
TEST_CASE("ReadAfterWriteShared",
  ...
  /** Write I/O **/
  metadata.resumeTime();
  FILE* write_fd = fopen(my_io_filename.c_str(), "w+");
  metadata.pauseTime();
  REQUIRE(write_fd != nullptr);
  ...
  /* Read I/O */
  metadata.resumeTime();
  FILE* read_fd = fopen(my_io_filename.c_str(), "r");
  metadata.pauseTime();
  REQUIRE(read_fd != nullptr);
